...
from gwd_numpy import GraphWassersteinDistance
...
def calculate_gwd(y_true, y_pred):
    ...
    pred_patch = picture_to_patch(y_pred).squeeze().cpu().numpy()
    gt_patch = picture_to_patch(y_true).squeeze().cpu().numpy()
    for i in range(pred_patch.shape[0]):
        x = torch.from_numpy(skeletonize(pred_patch[i,...]>0.5)).cuda()
        y = torch.from_numpy(skeletonize(gt_patch[i,...]>0.5)).cuda()
        gwd += gwd_loss(x,y)


    return gwd/ pred_patch.shape[0]


def calculate_metrics(y_true, y_pred):
    gwd = calculate_gwd(y_true, y_pred).item()
...
if __name__ == "__main__":
    ...
    """ Folders """
    model_name = '202310231553'
    create_dir(f"results/{model_name}")
    model_name1 = '202310231618'
    create_dir(f"results/{model_name1}")
    ...
    checkpoint_path1 = f"files/{model_name1}.pth"
    ...
    for i, (x, y) in tqdm(enumerate(zip(test_x, test_y)), total=len(test_x)):
        ...
        with torch.no_grad():
            ...
            score1 = calculate_metrics(y, pred_y1)
            jaccard = score[0] - score1[0]
            f1 = score[1] - score1[1]
            recall = score[2] - score1[2]
            precision = score[3] - score1[3]
            acc = score[4] - score1[4]
            gwd_score = score[5] - score1[5]
            hd95_score = score[6] - score1[6]
            dice_score = score[7] - score1[7]
            cldice_score =score[8] - score1[8]
            # print(
            #     f"Recall: {score[2]:1.4f} - Acc: {score[4]:1.4f} - Gwd : {score[5]} - hd95 :{score[6]:1.4f} - dice:{score[7]:1.4f}"
            #     f" - cldice_score: {score[8]:1.4f}")
            # print(
            #     f"Recall: {score1[2]:1.4f} - Acc: {score1[4]:1.4f} - Gwd : {score1[5]} - hd95 :{score1[6]:1.4f} - dice:{score1[7]:1.4f}"
            #     f" - cldice_score: {score1[8]:1.4f}")
            print(
                f"Recall: {recall:1.4f} - Acc: {acc:1.4f} - Gwd : {gwd_score} - hd95 :{hd95_score:1.4f} - dice:{dice_score:1.4f}"
            ...
            pred_y1 = pred_y1[0].cpu().numpy()  ## (1, 512, 512)
            pred_y1 = np.squeeze(pred_y1, axis=0)  ## (512, 512)
            pred_y1 = pred_y1 > 0.5
        ...
        cat_images = np.concatenate(
            [image, line, ori_mask, line, pred_y1 * 255], axis=1
        )
        cv2.imwrite(f"results/{model_name1}/{name}.png", cat_images)
